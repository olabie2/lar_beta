...
<div class="p-8">
    <div class="max-w-4xl">
        <h1 class="text-3xl font-bold text-white mb-6">Edit Career</h1>

        <form action="{{ route('admin.careers.update', $career) }}" method="POST" class="bg-gray-800 border border-gray-700 rounded-xl p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="title" class="block text-sm font-medium text-gray-300 mb-2">Job Title *</label>
                <input type="text" name="title" id="title" required value="{{ old('title', $career->title) }}" class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                @error('title')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" required class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description', $career->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="location" id="location" required value="{{ old('location', $career->location) }}" placeholder="e.g., Remote, New York, NY" class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    @error('location')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
