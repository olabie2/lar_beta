/**
 * FloatingLines - Vanilla JS implementation of Three.js wave shader animation
 */
class FloatingLines {
  constructor(container, options = {}) {
    this.container = container;
    this.options = {
      linesGradient: options.linesGradient || null,
      enabledWaves: options.enabledWaves || ['top', 'middle', 'bottom'],
      lineCount: options.lineCount || [10, 15, 20],
      lineDistance: options.lineDistance || [8, 6, 4],
  ...
  getLineCount(waveType) {
    const { lineCount, enabledWaves } = this.options;
  ...
  getLineDistance(waveType) {
    const { lineDistance, enabledWaves } = this.options;
  ...
  init() {
    const { enabledWaves } = this.options;
    ...
    const topLineCount = enabledWaves.includes('top') ? this.getLineCount('top') : 0;
    const middleLineCount = enabledWaves.includes('middle') ? this.getLineCount('middle') : 0;
    const bottomLineCount = enabledWaves.includes('bottom') ? this.getLineCount('bottom') : 0;

    const topLineDistance = enabledWaves.includes('top') ? this.getLineDistance('top') * 0.01 : 0.01;
    const middleLineDistance = enabledWaves.includes('middle') ? this.getLineDistance('middle') * 0.01 : 0.01;
    const bottomLineDistance = enabledWaves.includes('bottom') ? this.getLineDistance('bottom') * 0.01 : 0.01;
